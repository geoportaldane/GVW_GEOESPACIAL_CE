...
// 	"use strict";
	...
	let convertCoords = function(coords){
		let coordinate = toLonLat([coords[0],coords[1]], "EPSG:3857");
	...
	// ol.streetView = {
		...
		var StreetViewControl =  function(opt_options){
			...
			let pegmanDiv = document.createElement("div");
			pegmanDiv.className="pegman-button";
			options.content = pegmanDiv;
		...
		StreetViewControl.prototype = Object.create(Control.prototype);
		StreetViewControl.prototype.constructor = StreetViewControl;
		...
		function setStreetView(params, callback){
			...
			sv_service.getPanorama({
				...
				if(status === google.maps.StreetViewStatus.OK){
					...
					if(panorama !== undefined){
						...
						panorama.addListener("position_changed", function(){
							if(update){
								...
								let coords = fromLonLat([position.lng(),position.lat()], "EPSG:3857");
						...
						panorama.addListener("closeclick",function(){
							...
							panelToolbar[0].style.display = "block";
							let resultPanel = document.getElementsByClassName("result__container");
							resultPanel[0].style.display = "block";
					...
					alert("No hay información de StreetView para la zona");
				}
			...
			// sv.addListener('close-click',function(){
